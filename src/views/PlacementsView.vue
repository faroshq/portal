<template>
  ...
  <div>
    <BindLocation :workspace="getWorkspace()" />
  <div>
        ...
        <div class="px-4 py-4 -mx-4 overflow-x-auto sm:-mx-8 sm:px-8">
          ...
          >
            <table class="min-w-full leading-normal">
              ...
              <tbody v-if="placements != undefined">
                <tr v-for="(plc) in placements.items" :key="plc.metadata?.name">
                  <td
                  ...
                  <td class="px-5 py-5 text-sm bg-white border-b border-gray-200">
                    <div v-for="(key, label) in plc.spec.locationSelector" :key="key">
                  ...
                  <td class="px-5 py-5 text-sm bg-white border-b border-gray-200">
                        {{ plc.spec?.locationResource }}
                  </td>

                  <td class="px-5 py-5 text-sm bg-white border-b border-gray-200">
                        {{ plc.spec?.locationWorkspace }}
                  </td>
                  ...
                  >
                    <span class="relative">
                      <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800">
                        Phase: {{  plc.status?.phase }}
                      </span>
                      <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800">
                        Selected location: {{  plc.status?.selectedLocation }}
                      </span>
                      <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800">
                        Conditions: {{  plc.status?.conditions }}
                      </span>
                    </span>
...
import BindLocation from "../partials/orgs/BindLocation.vue";
...
export default defineComponent({
  ...
  methods: {
    ...
    deletePlacement(placement: V1alpha1Placement){
      const wl = {
        workspace: this.defaultWorkspace,
        placement: placement,
      }
      this.deletePlacementActions(wl)
    },
  }
})
