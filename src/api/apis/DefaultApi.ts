...
export class DefaultApiRequestFactory extends BaseAPIRequestFactory {
     ...
     * Register into Faros
     */
    public async register(_options?: Configuration): Promise<RequestContext> {
        let _config = _options || this.configuration;

        // Path Params
        const localVarPath = '/faros.sh/api/v1alpha1/oidc/register';
...
export class DefaultApiResponseProcessor {
     ...
     * @throws ApiException if the response code was not in [200, 299]
     */
     public async register(response: ResponseContext): Promise<V1alpha1User > {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("200", response.httpStatusCode)) {
            const body: V1alpha1User = ObjectSerializer.deserialize(
                ObjectSerializer.parse(await response.body.text(), contentType),
                "V1alpha1User", ""
            ) as V1alpha1User;
            return body;
