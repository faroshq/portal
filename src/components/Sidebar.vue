    ...
    >
      ...
      <nav class="mt-10">
        ...
        <p class="pl-4 my-2 text-xs font-semibold mb-4 text-gray-400">
          Workspaces {{ this.defaultOrganization.metadata?.name }}
        </p>

        <div v-for="(w) in this.getDefaultOrganizationWorkspaces()" :key="w">
          <router-link
          class="flex items-center px-6 py-2 mt-4 duration-200 border-l-4"
          :class="[$route.name === 'UIElements' ? activeClass : inactiveClass]"
          :to="`/organizations/${this.defaultOrganization.metadata?.name}/workspaces/${w.metadata?.name}`"
        >
          <svg
            class="w-5 h-5"
            viewBox="0 0 20 20"
        ...
        <router-link
          class="flex items-center px-6 py-2 mt-4 duration-200 border-l-4"
          :class="[$route.name === 'UIElements' ? activeClass : inactiveClass]"
          :to="`/organizations/${this.defaultOrganization.metadata?.name}/computes/compute`"
        >
          <svg
            class="w-5 h-5"
            viewBox="0 0 20 20"
